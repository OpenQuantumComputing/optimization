...
def gamma_beta_func_of_p(p, backend, M=5, K = 20, heuristic=False, weighted=False, num_shots=8192):
    ...
    for i in range(M):
        ...
        if heuristic:
            params, E = optimize_INTERP(K, G, backend, p, weighted, num_shots)
        else:
            params, E, temp = optimize_random(K, G, backend, p, weighted, num_shots)
        r = E/MAX_COST
...
def compare_methods(K, G, backend, p_max, weighted=False, num_shots=8192):
    """
    ...
    for i in range(len(p_list)):
        params_heur, E_heur_list[i] = optimize_INTERP(K, G, backend, p_list[i], weighted, num_shots)
        params_ran, E_ran_list[i], temp = optimize_random(K, G, backend, p_list[i], weighted, num_shots)
